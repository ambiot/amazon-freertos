...
#define BT_CONFIG_HEADER_LEN            6
    ...
    while (p_entry < config_buf + config_len)
    {
        ...
        {
        case 0x000c:
            if ((rltk_wlan_is_mp())||(!CHECK_SW(EFUSE_SW_IQK_HCI_OUT))) 
            {
...
uint8_t *hci_rtk_combine_config()
{
...
bool hci_board_init(void)
{
  ...
  HCI_PRINT_INFO1("hci_tp_open, this cut is AmebaZ2 %X CUT",0x0b);

  if (rltk_wlan_is_mp()) 
  {
      hci_board_debug("\r\n==========this is BT MP DRIVER===========,\r\n this cut is AMEBAZII B CUT\r\n");
  }
