...
SRAM_BF_DATA_SECTION u8 I2S_WAV_Buf[I2S_DMA_PAGE_SIZE];
...
void audio_play_sd_wav(u8* filename){
	...
	/* Read WAV header */
	res = f_read(&m_file, I2S_WAV_Buf, sizeof(WavHeader),(UINT*)&read_length);	
	if((res != FR_OK) || (read_length == 0)){
		printf("Read wav header fail!\n");
	}

	memcpy(&pwavHeader, I2S_WAV_Buf, sizeof(WavHeader));
	...
	res = f_read(&m_file, I2S_WAV_Buf, 8,(UINT*)&read_length);	
	if((res != FR_OK) || (read_length == 0)){
		printf("Read wav data info!\n");
	}
	if(strncmp(I2S_WAV_Buf, "data", 4)){
		printf("Have not found invalid data chunk\n");
		goto exit;
	}
 
	wav_length = *(u32*)&I2S_WAV_Buf[4];
	printf("Audio data total length 0x%08X\n", wav_length);

	do{
		/* Read a block */
		res = f_read(&m_file, I2S_WAV_Buf, I2S_DMA_PAGE_SIZE,(UINT*)&read_length);	
		if((res != FR_OK) || (read_length == 0)){
		...
		if(ptx_buf){
			if(read_length < I2S_DMA_PAGE_SIZE){
				...
				memcpy((void*)ptx_buf, (void*)&I2S_WAV_Buf[0], read_length);
			}else
				memcpy((void*)ptx_buf, (void*)&I2S_WAV_Buf[0], I2S_DMA_PAGE_SIZE);
...
#if defined(CONFIG_PLATFORM_8195A)
	i2s_init(&i2s_obj, I2S_SCLK_PIN, I2S_WS_PIN, I2S_SD_TX_PIN);
#else
...
{
	if(xTaskCreate(example_audio_thread, ((const char*)"example_audio_thread"), 512, NULL, tskIDLE_PRIORITY + 1, NULL) != pdPASS)
