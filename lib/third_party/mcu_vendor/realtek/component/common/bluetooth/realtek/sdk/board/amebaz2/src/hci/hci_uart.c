...
#define HCI_UART_RX_BUF_MASK        HCI_UART_RX_BUF_SIZE-1   /* Buffer size mask */
...
{
    /* Should neve happen */
    if (hci_adapter->rx_len >= HCI_UART_RX_BUF_SIZE)
    {
    ...
    hci_adapter->rx_write_idx &= HCI_UART_RX_BUF_MASK;
    hci_adapter->rx_len++;

    if (hci_adapter->rx_len >= HCI_UART_RX_DISABLE_COUNT && hci_adapter->rx_disabled == false)
    {
        hci_board_debug("uart_insert_char: rx disable, data len %u", (unsigned)hci_adapter->rx_len);
...
static void hciuart_irq(uint32_t id, SerialIrq event)
{
    ...
    if (event == RxIrq)
    {
        ...
        //  gtimer_reload(&my_timer2, DEFER_CHECK_US);
        do
        {
            ch = serial_getc(sobj);
...
uint16_t hci_uart_recv(uint8_t *p_buf, uint16_t size)
{
    ...
    if (rx_len)
    {
        ...
        p_uart_obj->rx_read_idx &= HCI_UART_RX_BUF_MASK;
        p_uart_obj->rx_len      -= rx_len;

        if (p_uart_obj->rx_disabled == true)    /* flow control */
        {
            if (p_uart_obj->rx_len < HCI_UART_RX_ENABLE_COUNT)
            {
