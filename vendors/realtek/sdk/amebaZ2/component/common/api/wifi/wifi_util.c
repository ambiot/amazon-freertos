...
/*this function used to close RF immediatly*/
int wext_close_lps_rf(const char *ifname)
{
	struct iwreq iwr;
	int ret = 0;
	__u16 pindex = 0;
	...
	// Encode parameters as TLV (type, length, value) format
	para = rtw_malloc( 7 + (1+1) );
	
	snprintf((char*)para, cmd_len, "pm_set");
	pindex = 7;

	para[pindex++] = 7; // cmd  only rtl8721d support
	para[pindex++] = 0; // len
...
#endif
extern void rtw_set_lowrssi_use_bg(int enable,int rssi);
