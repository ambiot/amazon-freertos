...
static int32_t lighting_lightness_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
    ...
    case LIGHT_LIGHTNESS_SERVER_GET_DEFAULT_TRANSITION_TIME:
        {
            generic_transition_time_p p_get_data = NULL;
            p_get_data = (generic_transition_time_p)pargs;
            if (p_get_data) {
                p_get_data->num_steps = lightness_transition_time.num_steps;
                p_get_data->step_resolution = lightness_transition_time.step_resolution;
            }
            data_uart_debug("light lightness server receive: num_steps %d, step_resolution %d \r\n", 
                            lightness_transition_time.num_steps, 
                            lightness_transition_time.step_resolution);
        }
        break;
...
static int32_t lighting_ctl_server_data(const mesh_model_info_p pmodel_info, uint32_t type, void *pargs)
{
    ...
    case LIGHT_CTL_SERVER_GET_DEFAULT_TRANSITION_TIME:
        {
            generic_transition_time_t *p_get_data = NULL;
            p_get_data = (generic_transition_time_t *)pargs;
            if (p_get_data) {
                p_get_data->num_steps = light_ctl_trans_time.num_steps;
                p_get_data->step_resolution = light_ctl_trans_time.step_resolution;
            }
            data_uart_debug("lighting ctl server receive: num_steps %d, step_resolution %d \r\n", 
                            light_ctl_trans_time.num_steps, 
                            light_ctl_trans_time.step_resolution);
        }
        break;
    ...
    case LIGHT_HSL_SERVER_GET_DEFAULT_TRANSITION_TIME:
        {
            ...
            p_get_data = (generic_transition_time_t *)pargs;
            if (p_get_data) {
                p_get_data->num_steps = light_hsl_trans_time.num_steps;
                p_get_data->step_resolution = light_hsl_trans_time.step_resolution;
            }
            data_uart_debug("lighting hsl server receive: num_steps %d, step_resolution %d \r\n", 
                            light_hsl_trans_time.num_steps, 
                            light_hsl_trans_time.step_resolution);
        }
        break;
    ...
    case LIGHT_XYL_SERVER_GET_DEFAULT_TRANSITION_TIME:
        {
            ...
            p_get_data = (generic_transition_time_t *)pargs;
            if (p_get_data) {
                p_get_data->num_steps = light_xyl_trans_time.num_steps;
                p_get_data->step_resolution = light_xyl_trans_time.step_resolution;
            }
            data_uart_debug("lighting xyl server receive: num_steps %d, step_resolution %d \r\n", 
                            light_xyl_trans_time.num_steps, 
                            light_xyl_trans_time.step_resolution);
        }
        break;
