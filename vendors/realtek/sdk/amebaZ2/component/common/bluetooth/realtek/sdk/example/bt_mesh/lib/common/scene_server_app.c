        ...
        case SCENE_SERVER_GET_DEFAULT_TRANSITION_TIME:
            {
				...
				if (p_get_data) {
					p_get_data->num_steps = scene_trans_time.num_steps;
					p_get_data->step_resolution = scene_trans_time.step_resolution;
				}
				data_uart_debug("scene server receive: transition time = step(%d), resolution(%d)\r\n", \
								scene_trans_time.num_steps, scene_trans_time.step_resolution);
            }
            break;
