...
/** @addtogroup  PERIPH_SEVER_CALLBACK Profile Server Callback Event Handler
    ...
    * @{
    */
T_SERVER_ID bt_airsync_config_srv_id;      /**< Wechat airsync service id*/
...
/** @defgroup  PERIPH_GAP_MSG GAP Message Handler
    ...
    * @{
    */
T_GAP_DEV_STATE bt_airsync_config_gap_dev_state = {0, 0, 0, 0, 0};                 /**< GAP device state */
T_GAP_CONN_STATE bt_airsync_config_gap_conn_state = GAP_CONN_STATE_DISCONNECTED; /**< GAP connection state */

uint8_t bt_airsync_config_conn_id = 0;
...
{
	uint8_t bt_addr[GAP_BD_ADDR_LEN];
	...
	memcpy(adv_data + 5, device_name, strlen((char const*)device_name));
	for(int i = 0; i < GAP_BD_ADDR_LEN; i ++) {
		memcpy(adv_data + 25 + i, bt_addr + 5 - i, 1);
	}
...
void airsync_handle_io_message(T_IO_MSG *p_wristband_msg)
{
    uint8_t msg_type = p_wristband_msg->subtype;
    switch (msg_type)
    {

    case IO_MSG_WAS_RX_VALUE:
        {
            uint8_t length = ((uint8_t *)(p_wristband_msg->u.buf))[0];
            uint8_t *p_data = (uint8_t *)p_wristband_msg->u.buf + 1;
            airsync_receive_data(p_data, length);
        }
        break;
...
void bt_airsync_config_app_handle_gap_msg(T_IO_MSG  *p_gap_msg);
/**
...
void bt_airsync_config_app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_BT_STATUS:
        {
            bt_airsync_config_app_handle_gap_msg(&io_msg);
        }
        break;
    case IO_MSG_TYPE_WRISTBNAD:
        {
            airsync_handle_io_message(&io_msg);
        }
        break;
 ...
 * @return   void
 */
void bt_airsync_config_app_handle_dev_state_evt(T_GAP_DEV_STATE new_state, uint16_t cause)
{
    APP_PRINT_INFO3("app_handle_dev_state_evt: init state %d, adv state %d, cause 0x%x",
                    new_state.gap_init_state, new_state.gap_adv_state, cause);
    if (bt_airsync_config_gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
    ...
    if (bt_airsync_config_gap_dev_state.gap_adv_state != new_state.gap_adv_state)
    {
    ...
    bt_airsync_config_gap_dev_state = new_state;
}
 ...
 * @return   void
 */
void bt_airsync_config_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
    APP_PRINT_INFO4("app_handle_conn_state_evt: conn_id %d old_state %d new_state %d, disc_cause 0x%x",
                    conn_id, bt_airsync_config_gap_conn_state, new_state, disc_cause);
    switch (new_state)
    {
    ...
    case GAP_CONN_STATE_CONNECTED:
        {
            ...
            bt_airsync_config_conn_id = conn_id;
        }
        break;
    ...
    bt_airsync_config_gap_conn_state = new_state;
}
 ...
 * @return   void
 */
void bt_airsync_config_app_handle_authen_state_evt(uint8_t conn_id, uint8_t new_state, uint16_t cause)
{
 ...
 * @return   void
 */
void bt_airsync_config_app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
 ...
 * @return   void
 */
void bt_airsync_config_app_handle_conn_param_update_evt(uint8_t conn_id, uint8_t status, uint16_t cause)
{
    switch (status)
    {
...
void bt_airsync_config_app_handle_gap_msg(T_IO_MSG *p_gap_msg)
{
    ...
    case GAP_MSG_LE_DEV_STATE_CHANGE:
        {
            bt_airsync_config_app_handle_dev_state_evt(gap_msg.msg_data.gap_dev_state_change.new_state,
    ...
    case GAP_MSG_LE_CONN_STATE_CHANGE:
        {
            bt_airsync_config_app_handle_conn_state_evt(gap_msg.msg_data.gap_conn_state_change.conn_id,
    ...
    case GAP_MSG_LE_CONN_MTU_INFO:
        {
            bt_airsync_config_app_handle_conn_mtu_info_evt(gap_msg.msg_data.gap_conn_mtu_info.conn_id,
    ...
    case GAP_MSG_LE_CONN_PARAM_UPDATE:
        {
            bt_airsync_config_app_handle_conn_param_update_evt(gap_msg.msg_data.gap_conn_param_update.conn_id,
    ...
    case GAP_MSG_LE_AUTHEN_STATE_CHANGE:
        {
            bt_airsync_config_app_handle_authen_state_evt(gap_msg.msg_data.gap_authen_state.conn_id,
...
T_APP_RESULT bt_airsync_config_app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    else if (service_id == bt_airsync_config_srv_id)
    {
        ...
        case SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION:
            {
                    ...
                    GATT_UUID_AIRSYNC_CHAR_INDICATION_ENABLE)
                {
                    ...
                    bt_airsync_config_app_send_msg_to_apptask(&enable_indicate_msg);
                }
                else
                {
                    //do nothing
