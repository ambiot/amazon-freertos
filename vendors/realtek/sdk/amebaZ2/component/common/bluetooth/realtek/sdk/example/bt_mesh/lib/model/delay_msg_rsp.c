...
uint32_t delay_msg_get_rsp_delay(uint16_t dst_addr)
{
    ...
    if (MESH_IS_UNICAST_ADDR(dst_addr))
    {
        ...
        real_delay %= DELAY_MSG_UNICAST_ADDR_MAX;
        if (real_delay < DELAY_MSG_UNICAST_ADDR_MIN)
        {
    ...
    else
    {
        ...
        real_delay %= DELAY_MSG_GROUP_ADDR_MAX;
        if (real_delay < DELAY_MSG_GROUP_ADDR_MIN)
        {
